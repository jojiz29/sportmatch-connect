      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            {COURTS.map((c) => (
              <button
                key={c.id}
                onClick={() => { setSelected(c); setConfirmed(false); }}
                className={`text-left rounded-2xl overflow-hidden border transition-all ${
                  selected.id === c.id ? "border-primary ring-glow" : "border-border hover:border-accent"
                }`}
              >
                <div className="relative h-40">
                  ...
                  <div className="absolute top-3 right-3">
                    {c.available ? (
                      <span className="px-2 py-1 rounded-full bg-neon/90 text-neon-foreground text-xs font-semibold">Disponible</span>
                    ) : (
                      <span className="px-2 py-1 rounded-full bg-destructive/90 text-destructive-foreground text-xs">Ocupado</span>
                    )}
                  </div>
                  <div className="absolute bottom-3 left-3 right-3">
                    <div className="font-semibold">{c.name}</div>
                    <div className="text-xs text-white/80 flex items-center gap-2 mt-0.5">
                      <span className="flex items-center gap-1"><MapPin className="h-3 w-3" />{c.distanceKm} km</span>
                      <span className="flex items-center gap-1"><Star className="h-3 w-3 fill-warning text-warning" />{c.rating}</span>
                      <span>· ${c.pricePerHour}/h</span>
                    </div>
                  </div>
                </div>
                <div className="p-3 bg-card">
                  <div className="flex flex-wrap gap-1">
                    {c.amenities.map((a) => (
                      <span key={a} className="text-xs px-2 py-0.5 rounded-full bg-accent">{a}</span>
                    ))}
                  </div>
                </div>
              </button>
        ...
        <div className="space-y-4">
          <div className="bg-gradient-card border border-border rounded-2xl p-5 shadow-card">
            ...
            <p className="text-xs text-muted-foreground">{selected.sport} · ${selected.pricePerHour}/h</p>
            ...
            <div className="mt-5 pt-4 border-t border-border space-y-2 text-sm">
              <Row label="Cancha" value={`$${selected.pricePerHour}`} />
              <Row label="Servicio" value="$3" />
              <Row label="Dividir entre" value="4 jugadores" />
              <div className="flex justify-between font-semibold pt-2 border-t border-border">
                <span>Tu parte</span>
                <span className="text-neon">${((selected.pricePerHour + 3) / 4).toFixed(2)}</span>
              </div>
            </div>
