...
import {
  Activity, HeartPulse, Zap, Watch, ArrowUpRight, ArrowDownRight, Clock, RefreshCw, Flame,
} from "lucide-react";
  ...
  useEffect(() => {
    ...
    if (useAuthStore.getState().isDemoMode) {
      ...
      if (storedAttendance) setAttendanceDays(JSON.parse(storedAttendance));
        ...
        localStorage.setItem(`sportmatch_demo_attendance_${user.id}`, JSON.stringify(mockAttendance));
      }
    } else {
      const fetchStats = async () => {
        try {
          const { data: stats } = await supabase.from("user_stats").select("*").eq("user_id", user.id).maybeSingle();
          setStreak(stats ? { current_streak: stats.current_streak, max_streak: stats.max_streak } : { current_streak: 0, max_streak: 0 });

          const { data: participants } = await supabase
            .from("match_participants").select("joined_at, matches(date)").eq("user_id", user.id).eq("status", "ATTENDED");
          if (participants) {
            const days = (participants as unknown as { joined_at: string; matches: { date: string } | null }[])
              .map((p) => p.matches?.date || p.joined_at?.split("T")[0]).filter(Boolean);
            setAttendanceDays(days);
          }
        } catch (err) { console.error("Error fetching user stats/attendance in IoT:", err); }
      };
      fetchStats();
    }
  }, [user]);
  ...
  const handleToggleAppleHealth = (checked: boolean) => {
    if (checked) {
      ...
      setTimeout(() => {
        ...
        setHeartRate(145); setCalories(580); setDuration(50);
        setSteps(6400); setDistanceKm(3.0);
        setHistory((prevHist) => [...prevHist.slice(1), 145]);
        setIsSyncing(false);
        toast.success("¡Apple Health sincronizado con éxito!", { id: toastId, description: "Métricas obtenidas: 50 mins, 580 Kcal, 6,400 pasos." });
      }, 1500);
    } else {
    ...
    setTimeout(() => {
      const mockPayload = {
        ...
        activity: {
          ...
          elapsed_time_min: 75, distance_km: 4.1, steps_count: 8200,
          average_heart_rate: 138, calories_burned: 720,
        },
      };
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-center justify-between relative z-10">
              <div>
                ...
                <div className="text-6xl font-black tracking-tighter">
                  {heartRate} <span className="text-2xl text-muted-foreground font-medium">bpm</span>
                </div>
              </div>
            ...
            <div className="mt-8 flex items-end gap-2 h-32 relative z-10">
              {history.map((v: number, i: number) => (
                <div key={i} className="flex-1 flex flex-col justify-end items-center group">
                  <div className="w-full rounded-t-lg bg-gradient-to-t from-destructive/20 to-destructive transition-all duration-500 group-hover:opacity-80"
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="bg-gradient-card border border-border rounded-2xl p-5">
              <div className="flex items-center gap-2 text-warning mb-4"><Zap className="h-5 w-5" /><span className="font-semibold uppercase text-sm">Calorías Activas</span></div>
              <div className="text-4xl font-bold">{calories} <span className="text-lg text-muted-foreground">kcal</span></div>
              <div className="mt-4 flex items-center gap-2 text-xs text-neon"><ArrowUpRight className="h-4 w-4" /> +15% vs promedio</div>
            </div>
            <div className="bg-gradient-card border border-border rounded-2xl p-5">
              <div className="flex items-center gap-2 text-electric mb-4"><Clock className="h-5 w-5" /><span className="font-semibold uppercase text-sm">Duración Activa</span></div>
              <div className="text-4xl font-bold">{duration} <span className="text-lg text-muted-foreground">min</span></div>
              <div className="mt-4 flex items-center gap-2 text-xs text-neon"><ArrowUpRight className="h-4 w-4" /> En zona de quema de grasa</div>
            </div>
            <div className="bg-gradient-card border border-border rounded-2xl p-5">
              <div className="flex items-center gap-2 text-teal-400 mb-4"><Activity className="h-5 w-5" /><span className="font-semibold uppercase text-sm">Pasos Totales</span></div>
              <div className="text-4xl font-bold">{steps} <span className="text-lg text-muted-foreground">pasos</span></div>
              <div className="mt-4 flex items-center gap-2 text-teal-400 text-xs"><ArrowUpRight className="h-4 w-4" /> Ritmo de match constante</div>
            </div>
            <div className="bg-gradient-card border border-border rounded-2xl p-5">
              <div className="flex items-center gap-2 text-neon mb-4"><Activity className="h-5 w-5" /><span className="font-semibold uppercase text-sm">Distancia Match</span></div>
              <div className="text-4xl font-bold">{distanceKm.toFixed(1)} <span className="text-lg text-muted-foreground">km</span></div>
              <div className="mt-4 flex items-center gap-2 text-destructive text-xs"><ArrowDownRight className="h-4 w-4" /> -2% vs promedio</div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-card border border-border rounded-2xl p-6 shadow-card">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2"><Flame className="h-5 w-5 text-neon" /><h3 className="font-bold text-sm">{t("onboarding.streak_title", "Racha Deportiva")}</h3></div>
              {streak && (
                <div className="text-[10px] text-muted-foreground flex items-center gap-1.5">
                  <span>{t("onboarding.streak_current", "Racha actual:")} <strong className="text-neon">{streak.current_streak}</strong> {streak.current_streak === 1 ? t("onboarding.streak_unit_sing", "sem") : t("onboarding.streak_unit_plur", "sems")}</span>
                  <span>·</span>
                  <span>{t("onboarding.streak_max", "Máx:")} <strong className="text-foreground">{streak.max_streak}</strong></span>
                </div>
              )}
            </div>
            <div className="flex flex-col items-center gap-3">
              <div className="relative">
                <svg width="220" height="110" viewBox="0 0 220 110" className="text-muted-foreground">
                  <text x="5" y="18" fontSize="8" fill="currentColor">{t("onboarding.day_mon", "Lun")}</text>
                  <text x="5" y="46" fontSize="8" fill="currentColor">{t("onboarding.day_wed", "Mié")}</text>
                  <text x="5" y="74" fontSize="8" fill="currentColor">{t("onboarding.day_fri", "Vie")}</text>
                  <text x="5" y="102" fontSize="8" fill="currentColor">{t("onboarding.day_sun", "Dom")}</text>
                  {contributionGrid.map((cell, idx) => {
                    const isAttended = attendanceDays.includes(cell.date);
                    return (
                      <rect key={idx} x={35 + cell.col * 36} y={10 + cell.row * 14} width="10" height="10" rx="2"
                        fill={isAttended ? "#39FF14" : "#1e293b"}
                        stroke={isAttended ? "#39FF14" : "rgba(255,255,255,0.05)"} strokeWidth="0.5"
              ...
              <p className="text-[10px] text-muted-foreground text-center">
                {t("onboarding.streak_footer_prefix", "Los días con asistencia verificada se iluminan en")}{" "}
                <span className="text-[#39FF14] font-bold">{t("onboarding.streak_footer_color", "Verde Neón")}</span>.
              </p>
            </div>
          </div>
          ...
          <div className="bg-gradient-card border border-border rounded-2xl p-6">
            <h3 className="font-semibold mb-6 flex items-center gap-2">
              <div className="h-2 w-2 rounded-full bg-neon animate-pulse" /> Sincronización de Dispositivos (Health Kit)
            </h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-card">
                <div className="flex items-center gap-3">
                  <Watch className={`h-6 w-6 ${appleHealthConnected ? "text-neon" : "text-muted-foreground"}`} />
                  <div>
                    <div className="font-medium text-sm">Apple Health Kit</div>
                    <div className={`text-xs ${appleHealthConnected ? "text-neon font-semibold" : "text-muted-foreground"}`}>
                      {appleHealthConnected ? "Conectado · Transmitiendo" : "Desconectado"}
                    </div>
                  </div>
                </div>
                <input type="checkbox" checked={appleHealthConnected} disabled={isSyncing} onChange={(e) => handleToggleAppleHealth(e.target.checked)}
                  className="w-4 h-4 cursor-pointer accent-neon" />
              </div>
              <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-card">
                <div className="flex items-center gap-3">
                  <Activity className={`h-6 w-6 ${stravaConnected ? "text-orange-500" : "text-muted-foreground"}`} />
                  <div>
                    <div className="font-medium text-sm">Strava Webhook API</div>
                    <div className={`text-xs ${stravaConnected ? "text-orange-500 font-semibold" : "text-muted-foreground"}`}>
            ...
            <button onClick={handleSimulateWebhook} disabled={isSyncing}
              ...
              <RefreshCw className={`h-4 w-4 ${isSyncing ? "animate-spin" : ""}`} /> Simular Webhook (Strava/Apple)
            </button>
          </div>
        </div>
      </div>
