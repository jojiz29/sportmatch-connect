...
const ALLOWED_DOMAINS = [
  ...
  "icloud.com", "protonmail.com", "sportmatch.app", "puka.com",
];
...
function getEmailValidationError(email: string, t: (key: string) => string): string | null {
  ...
  const isWhitelisted = ALLOWED_DOMAINS.some((allowed) => domain === allowed || domain.endsWith("." + allowed));
...
function getPasswordStrength(criteria: { length: boolean; upper: boolean; lower: boolean; number: boolean; special: boolean }) {
...
function RegisterPage() {
    ...
    if (typeof window !== "undefined" && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          ...
          setLat(-12.14); setLng(-76.995);
        },
      );
    } else { setLat(-12.14); setLng(-76.995); }
  }, []);
  ...
  const { values, handleChange, handleBlur, handleSubmit, isSubmitting } = useStrictForm({
    initialValues: {
      fullName: "",
      companyName: "",
      category: "Tienda" as "Canchas" | "Gym" | "Academia" | "Tienda" | "Nutricionista" | "Fisioterapia" | "Torneos" | "Marcas" | "Patrocinador" | "Bebidas",
      email: "",
      password: "",
    },
    validate: (vals) => {
      ...
      if (!Object.values(criteria).every(Boolean)) errors.password = "La contraseña no cumple con todos los requisitos.";
      if (role === "PLAYER" && !vals.fullName.trim()) errors.fullName = "El nombre completo es requerido.";
      if (role === "BUSINESS" && !vals.companyName.trim()) errors.companyName = "El nombre de la empresa es requerido.";
      return errors;
    },
    onSubmit: async (vals) => {
      const newUser = {
        ...
        bio: role === "BUSINESS" ? t("register.business_bio_template", { companyName: vals.companyName }) : t("profile.placeholder_bio") || t("register.default_player_bio"),
        trust_score: 50,
        fitcoins_balance: 0,
  ...
  const isPlayerStep1Valid = values.fullName.trim().length > 0 && values.email.length > 0 && emailError === null && isPasswordValid;
  const isFormValid = (role === "PLAYER" ? isPlayerStep1Valid : values.companyName.trim().length > 0) && values.email.length > 0 && emailError === null && isPasswordValid;
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-lg bg-gradient-card border border-border rounded-3xl p-8 shadow-card backdrop-blur-md">
        <div className="text-center mb-6">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight bg-gradient-neon bg-clip-text text-transparent">{t("register.title")}</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {role === "PLAYER" ? (
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" required name="fullName" value={values.fullName} onChange={handleChange} onBlur={handleBlur}
            ...
            <>
              <div>
                <label className="text-sm font-semibold mb-2 block">{t("register.companyName")}</label>
                <div className="relative">
                  <UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                  <input type="text" required name="companyName" value={values.companyName} onChange={handleChange} onBlur={handleBlur}
                    className="w-full pl-12 pr-4 py-3 bg-background/50 border border-border rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all"
                    placeholder={t("register.placeholder_company") || "Ej. Deportes Flores S.A.C."} />
                </div>
              </div>
              <div>
                <label className="text-sm font-semibold mb-2 block">{t("register.businessCategory")}</label>
              ...
              <div>
                <label className="text-sm font-semibold mb-2 block">{t("register.gps_location")}</label>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <span className="text-[10px] text-muted-foreground block mb-1">{t("register.latitude")}</span>
                    <input type="number" step="any" required value={lat || ""} onChange={(e) => setLat(parseFloat(e.target.value))}
                      className="w-full px-4 py-2 bg-background/50 border border-border rounded-xl focus:ring-2 focus:ring-primary outline-none transition-all text-sm" placeholder={t("register.latitude") || "Latitud"} />
                  </div>
                  <div>
                    <span className="text-[10px] text-muted-foreground block mb-1">{t("register.longitude")}</span>
                    <input type="number" step="any" required value={lng || ""} onChange={(e) => setLng(parseFloat(e.target.value))}
                      className="w-full px-4 py-2 bg-background/50 border border-border rounded-xl focus:ring-2 focus:ring-primary outline-none transition-all text-sm" placeholder={t("register.longitude") || "Longitud"} />
                  </div>
                </div>
              </div>
            </>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="email" required name="email" value={values.email} onChange={handleChange} onBlur={handleBlur}
            ...
            {emailError && <p className="text-[11px] text-red-500 mt-1.5 ml-1 animate-fade-in">{emailError}</p>}
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type={showPassword ? "text" : "password"} required name="password" value={values.password} onChange={handleChange} onBlur={handleBlur}
              ...
              <button type="button" onClick={() => setShowPassword(!showPassword)}
                className="absolute right-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground hover:text-foreground focus:outline-none transition-colors" tabIndex={-1}>
            ...
            {values.password && (
              <div className="mt-3 space-y-2 animate-fade-in">
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className={`font-semibold ${strength === "weak" ? "text-red-500" : strength === "medium" ? "text-yellow-500" : "text-green-500"}`}>
                    {t(`auth.password_strength_${strength}`)}
                  </span>
                </div>
                <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
                  <div className={`h-full transition-all duration-300 ${strength === "weak" ? "bg-red-500" : strength === "medium" ? "bg-yellow-500" : "bg-green-500"}`} style={{ width: `${strengthPercent}%` }} />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-1 text-[11px] mt-2">
                  <div className="flex items-center gap-1.5">
                    <span className={`h-4 w-4 rounded-full flex items-center justify-center text-[10px] font-bold ${criteria.length ? "bg-green-500/20 text-green-500" : "bg-muted text-muted-foreground"}`}>{criteria.length ? "✓" : "○"}</span>
                    <span className={criteria.length ? "text-foreground" : "text-muted-foreground"}>{t("auth.password_crit_len")}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className={`h-4 w-4 rounded-full flex items-center justify-center text-[10px] font-bold ${criteria.upper ? "bg-green-500/20 text-green-500" : "bg-muted text-muted-foreground"}`}>{criteria.upper ? "✓" : "○"}</span>
                    <span className={criteria.upper ? "text-foreground" : "text-muted-foreground"}>{t("auth.password_crit_upper")}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className={`h-4 w-4 rounded-full flex items-center justify-center text-[10px] font-bold ${criteria.lower ? "bg-green-500/20 text-green-500" : "bg-muted text-muted-foreground"}`}>{criteria.lower ? "✓" : "○"}</span>
                    <span className={criteria.lower ? "text-foreground" : "text-muted-foreground"}>{t("auth.password_crit_lower")}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className={`h-4 w-4 rounded-full flex items-center justify-center text-[10px] font-bold ${criteria.number ? "bg-green-500/20 text-green-500" : "bg-muted text-muted-foreground"}`}>{criteria.number ? "✓" : "○"}</span>
                    <span className={criteria.number ? "text-foreground" : "text-muted-foreground"}>{t("auth.password_crit_number")}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className={`h-4 w-4 rounded-full flex items-center justify-center text-[10px] font-bold ${criteria.special ? "bg-green-500/20 text-green-500" : "bg-muted text-muted-foreground"}`}>{criteria.special ? "✓" : "○"}</span>
                    <span className={criteria.special ? "text-foreground" : "text-muted-foreground"}>{t("auth.password_crit_special")}</span>
                  </div>
                </div>
              </div>
            )}
          ...
          <button type="submit" disabled={(role === "PLAYER" ? !isPlayerStep1Valid : !isFormValid) || isSubmitting}
          ...
          <div className="relative my-4 flex items-center justify-center">
            ...
            <span className="relative bg-background px-3 text-xs text-muted-foreground uppercase font-semibold">{t("auth.or_continue_with", "O continúa con")}</span>
          </div>
          ...
          <button type="button" onClick={handleGoogleLogin}
            ...
            <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none">
              ...
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.53 6.9-4.53z" fill="#EA4335" />
            </svg>
        ...
        <div className="mt-6 text-center">
          <a href="/login" className="text-sm text-primary hover:underline transition-all">{t("login.toggle_signin")}</a>
        </div>
      </div>
    </div>
  );
