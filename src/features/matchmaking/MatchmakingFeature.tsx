...
import {
  X, Info, MapPin, Sparkles, MessageSquare, Shield, TrendingUp,
  Users, Swords, Clock3, Check,
} from "lucide-react";
...
import {
  PlayerChallenge, getPendingChallengesSent, getPendingChallengesReceived,
  createPlayerChallenge, respondToPlayerChallenge,
} from "@/shared/api/challengeService";
import {
  getPlayerConnections, createPlayerConnection, PlayerConnection,
...
export function MatchmakingFeature({ initialStack }: { initialStack: User[] }) {
    ...
    backendApi.matches
      ...
      .catch(() => {
        apiClient.matches
          ...
          .then((data) => { if (active) setInspectedUserMatches(data); })
          .catch((err) => console.error("Error loading inspected user matches:", err));
      })
      .finally(() => { if (active) setIsLoadingMatches(false); });
    return () => { active = false; };
  }, [inspectedUser]);
      ...
      .then((challenges) => {
        if (active) {
          setPendingChallengeUserIds(
            challenges.filter((c) => activeSport === "Todos" || c.sport === activeSport)
  ...
  useEffect(() => {
    ...
    getPendingChallengesReceived(currentUser.id)
      .then((challenges) => { if (active) setReceivedChallenges(challenges || []); })
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", {
        event: "INSERT", schema: "public", table: "player_connections",
        ...
        const matched = stack.find((c) => c.id === incomingUserId) ||
          (await apiClient.users.getMatches()).find((c) => c.id === incomingUserId);
        if (matched) { setMatchedUser(matched); setMatchMessage(""); }
      })
      .subscribe();
  ...
  const handleConnect = async (target: User) => {
    ...
    try {
      ...
      const result = await createPlayerConnection({
        ...
        sport: recommendation?.matchedSport ?? (activeSport === "Todos" ? target.preferred_sports?.[0] : activeSport),
      ...
      toast.error("No se pudo guardar la conexión", {
        description: error instanceof Error && error.message.includes("límite")
  ...
  const handleChallengeResponse = async (challenge: PlayerChallenge, decision: "accepted" | "rejected") => {
    if (!currentUser) return;
    try {
      ...
      if (decision === "accepted") {
        ...
        await chatStore.sendMessage(chatId, `Acepté tu desafío de ${challenge.sport}. Coordinemos el partido.`);
        chatStore.setActiveConversation(chatId);
        toast.success("Desafío aceptado", { description: "Abrimos el chat para coordinar el partido." });
    ...
    } finally { setResolvingChallengeId(null); }
  };
    ...
    <div className="grid lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2 flex justify-center">
        <div className="relative w-full max-w-md h-[560px]">
          {isLoading ? (
            ...
            <AnimatePresence>
              {(rankedStack || []).slice(0, 3).reverse().map((p, i, arr) => {
                const idx = arr.length - 1 - i;
                const isTop = idx === 0;
                const dist = baseLocation && p.user.last_location_lat && p.user.last_location_lng
                  ? calculateDistance(baseLocation.lat, baseLocation.lng, p.user.last_location_lat, p.user.last_location_lng)
                  : p.distanceKm || 0;
                ...
                return (
                  <motion.div
                    ...
                    style={{ transform: `translateY(${idx * 12}px) scale(${1 - idx * 0.03})`, zIndex: 10 - idx, opacity: 1 - idx * 0.1 }}
                  ...
                  >
                    ...
                    <div className="flex justify-center items-center py-4 relative z-10">
                      ...
                      <div className="h-[130px] w-[130px] rounded-full p-[3px] relative flex items-center justify-center"
                        style={{ background: "linear-gradient(135deg, #FF007F, #FF6B35, #7B2CBF)" }}>
                        <div className="h-full w-full rounded-full overflow-hidden border-2 border-background bg-muted">
                          <Link to="/app/profile/$userId" params={{ userId: p.user.id }} className="block h-full w-full">
                            <img src={p.user.avatar_url} alt={p.user.name} className="w-full h-full object-cover" />
                          </Link>
                        </div>
                        <span className="absolute bottom-1.5 right-1.5 h-5 w-5 rounded-full bg-[#39FF14] border-[3px] border-[#0D152D] flex items-center justify-center"
                    ...
                    <div className="px-5 pb-2 space-y-2 z-10 relative">
                      ...
                      {(() => {
                        ...
                        const displayLevel =
                          sportLevel === "Amateur" ? t("skills.amateur", "Amateur") :
                          sportLevel === "Intermediate" || sportLevel === "Intermedio" ? t("skills.intermedio", "Intermedio") :
                          sportLevel === "Advanced" || sportLevel === "Avanzado" ? t("skills.avanzado", "Avanzado") :
                          sportLevel === "Pro" ? t("skills.pro", "Pro") : sportLevel;
                        return (
                      ...
                      <div className="flex flex-wrap justify-center gap-1 pt-0.5">
                        {(p.user.trust_score || 0) >= 90 && (
                          <span className="text-[9px] px-2 py-0.5 rounded bg-[#39FF14]/10 border border-[#39FF14]/15 text-[#39FF14]/80 font-semibold">#Confiable</span>
                        )}
                        {(p.user.trust_score || 0) >= 80 && (
                          <span className="text-[9px] px-2 py-0.5 rounded bg-muted border border-border text-muted-foreground font-semibold">#BuenNivel</span>
                        )}
                        <span className="text-[9px] px-2 py-0.5 rounded bg-muted border border-border text-muted-foreground font-semibold">#Puntual</span>
                      </div>
                    </div>
                    ...
                    {isTop && (
                      <div className="flex items-center justify-center gap-5 px-5 pt-3 pb-5 border-t border-border/40 mt-auto">
                        <button onClick={() => swipe(p.user.id, "pass")}
                        ...
                        <button onClick={() => setInspectedUser(p.user)}
                        ...
                        <button onClick={() => void handleConnect(p.user)} disabled={isSavingConnection}
        ...
        <div className="bg-gradient-card border border-border rounded-2xl p-5">
          ...
          {topRecommendation && (
            <div className="mt-4 space-y-3">
              <Bar label={topRecommendation.matchedSport || activeSport} value={topRecommendation.breakdown.sport} />
              <Bar label={t("matchmaking.level")} value={topRecommendation.breakdown.level} />
              <Bar label={t("matchmaking.schedule")} value={topRecommendation.breakdown.availability} />
          ...
          <div className="bg-gradient-card border border-border rounded-2xl p-5">
            ...
            <div className="mt-4 space-y-3">
              {(receivedChallenges || []).map((challenge) => {
                const challenger = challenge.challenger || stack.find((c) => c.id === challenge.challenger_id);
                const isResolving = resolvingChallengeId === challenge.id;
                return (
                  <div key={challenge.id} className="rounded-2xl border border-border/70 bg-background/50 p-3">
                    <div className="flex items-center gap-3">
                      <img src={challenger?.avatar_url || ""} alt={challenger?.name || "Jugador"}
                        className="h-10 w-10 rounded-full bg-muted object-cover" />
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-sm truncate">{challenger?.name || "Jugador"}</div>
                    ...
                    {challenge.message && (
                      <p className="mt-3 text-xs text-muted-foreground italic leading-relaxed">“{challenge.message}”</p>
                    )}
                    <div className="grid grid-cols-2 gap-2 mt-3">
                      <button onClick={() => handleChallengeResponse(challenge, "rejected")} disabled={isResolving}
                      ...
                      <button onClick={() => handleChallengeResponse(challenge, "accepted")} disabled={isResolving}
                        ...
                        {isResolving ? <Clock3 className="h-3.5 w-3.5 animate-pulse" /> : <Check className="h-3.5 w-3.5" />}
                        Aceptar
                      </button>
                    </div>
                  </div>
        ...
        {challengeTarget && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="absolute inset-0 bg-background/90 backdrop-blur-md" onClick={() => setChallengeTarget(null)} />
            <motion.div initial={{ opacity: 0, scale: 0.94, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              ...
              <button onClick={() => setChallengeTarget(null)}
                className="absolute top-4 right-4 h-8 w-8 rounded-full bg-muted hover:bg-accent grid place-items-center cursor-pointer" aria-label="Cerrar desafío">
                <X className="h-4 w-4" />
              </button>
              <div className="flex items-center gap-3 pr-10">
                ...
                <div>
                  <div className="text-[10px] text-primary font-bold uppercase tracking-wider">Nuevo desafío</div>
              ...
              <div className="mt-5 rounded-2xl border border-border bg-background/50 p-4 flex items-center gap-3">
                <img src={challengeTarget.avatar_url} alt={challengeTarget.name} className="h-12 w-12 rounded-full bg-muted object-cover" />
                <div className="flex-1 min-w-0">
                  <div className="font-semibold truncate">{challengeTarget.name}</div>
                  <div className="text-xs text-muted-foreground mt-1">{challengeSport} · Nivel {challengeTarget.level}</div>
                </div>
                <span className="px-2.5 py-1 rounded-full bg-neon/10 border border-neon/20 text-neon text-xs font-bold">
                  {topRecommendation?.user.id === challengeTarget.id ? `${topRecommendation.score}% match` : challengeSport}
                </span>
              </div>
              <label className="block mt-5">
                <span className="text-xs font-semibold">Mensaje opcional</span>
                <textarea value={challengeMessage} onChange={(e) => setChallengeMessage(e.target.value)}
                ...
                <span className="block text-right text-[10px] text-muted-foreground mt-1">{challengeMessage.length}/240</span>
              </label>
              <div className="grid grid-cols-2 gap-3 mt-5">
                <button onClick={() => setChallengeTarget(null)}
                  className="py-3 rounded-xl border border-border bg-muted/60 text-sm font-semibold hover:bg-muted cursor-pointer">Cancelar</button>
                <button onClick={handleSendChallenge} disabled={isSendingChallenge}
                  ...
                  {isSendingChallenge ? <><Clock3 className="h-4 w-4 animate-pulse" /> Enviando...</> : <><Swords className="h-4 w-4" /> Enviar desafío</>}
                </button>
              </div>
            </motion.div>
          </div>
      ...
      <AnimatePresence>
        {matchedUser && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="absolute inset-0 bg-background/90 backdrop-blur-md" onClick={() => setMatchedUser(null)} />
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }}
              ...
              <div className="text-neon text-xs font-bold uppercase tracking-wider mb-2 animate-pulse">{t("matchmaking.its_a_match")}</div>
              <h2 className="text-3xl font-extrabold bg-gradient-neon bg-clip-text text-transparent mb-6">{t("matchmaking.sport_connection")}</h2>
              <div className="flex items-center justify-center gap-6 mb-8 relative">
                <div className="relative">
                  <img src={currentUser?.avatar_url} alt="" className="h-24 w-24 rounded-full bg-muted object-cover border-4 border-primary shadow-glow animate-bounce-slow" />
                  <span className="absolute -bottom-2 right-2 px-2 py-0.5 rounded-full bg-primary text-white text-[10px] font-bold">{t("matchmaking.me")}</span>
                </div>
                ...
                <div className="relative">
                  <img src={matchedUser.avatar_url} alt="" className="h-24 w-24 rounded-full bg-muted object-cover border-4 border-neon shadow-neon animate-bounce-slow delay-150" />
                  <span className="absolute -bottom-2 left-2 px-2 py-0.5 rounded-full bg-neon text-neon-foreground text-[10px] font-bold">{matchedUser.name}</span>
                </div>
              </div>
              <p className="text-sm text-muted-foreground mb-6">
                {t("matchmaking.match_desc", { name: matchedUser.name, sport: matchedUser.preferred_sports?.[0] || t("matchmaking.sport_default") })}
              </p>
              <textarea value={matchMessage} onChange={(e) => setMatchMessage(e.target.value)} maxLength={240} rows={3}
              ...
              <div className="flex flex-col gap-2">
                <button onClick={async () => {
                  try {
                    ...
                    await chatStore.sendMessage(chatId, matchMessage.trim() || `Hola ${matchedUser.name}, ¿coordinamos para jugar?`);
                    chatStore.setActiveConversation(chatId);
                    setMatchedUser(null);
                    navigate({ to: "/app/chat" });
                  } catch (error) {
                    console.error("[chat] match-greeting:error", { targetUserId: matchedUser.id, error });
                ...
                <button onClick={async () => {
                  const chatStore = useChatStore.getState();
                  const chatId = await chatStore.createChat(matchedUser.id);
                  if (!chatId) { toast.error("No se pudo abrir el chat del match"); return; }
                  chatStore.setActiveConversation(chatId);
                  setMatchedUser(null);
                  navigate({ to: "/app/chat" });
                ...
                <button onClick={() => setMatchedUser(null)}
      ...
      <AnimatePresence>
        {inspectedUser && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="absolute inset-0 bg-background/90 backdrop-blur-md" onClick={() => setInspectedUser(null)} />
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.9, opacity: 0, y: 50 }}
              ...
              <button onClick={() => setInspectedUser(null)}
              ...
              <div className="flex flex-wrap md:flex-nowrap items-start gap-6 relative mb-6">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground flex items-center gap-1 mt-1">
                    <MapPin className="h-3 w-3" />{inspectedUser.city} · {t("profile.age_label", { age: inspectedUser.age })}
                  </p>
                  <p className="text-sm mt-2 font-light">{inspectedUser.bio || t("register.default_player_bio")}</p>
                  <div className="flex flex-wrap gap-2 mt-3">
                    {inspectedUser.preferred_sports?.map((s) => (
                      <span key={s} className="px-3 py-1 rounded-full bg-violet/20 text-xs border border-violet/30">{s}</span>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <Stat icon={<TrendingUp className="h-4 w-4 text-electric" />} label={t("profile.matches")} value={inspectedUser.matches_played ?? 0} />
                <Stat icon={<Shield className="h-4 w-4 text-neon" />} label={t("profile.trust_score")} value={`${inspectedUser.trust_score ?? 0}%`} />
                <Stat icon={<Users className="h-4 w-4 text-neon" />} label={t("profile.followers")} value={inspectedUser.followers_count ?? 0} />
                <Stat icon={<Users className="h-4 w-4 text-electric" />} label={t("profile.following")} value={inspectedUser.following_count ?? 0} />
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="glass rounded-2xl p-4">
                  ...
                  <div className="mt-2 text-center">
                    <div className="text-4xl font-extrabold text-gradient">{inspectedUser.trust_score ?? 0}%</div>
                    <div className="text-xs text-muted-foreground mt-1">
                      {(inspectedUser.trust_score || 0) >= 90 ? t("profile.trust_level_excellent") :
                       (inspectedUser.trust_score || 0) >= 70 ? t("profile.trust_level_good") : t("profile.trust_level_risk")}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 max-h-[200px] overflow-y-auto pr-1">
                    {isLoadingMatches ? (
                      ...
                      (inspectedUserMatches || []).map((m) => (
                        <div key={m.id} className="flex items-center gap-3 text-xs p-2 rounded-xl hover:bg-accent/50 transition-colors border border-border/20">
                          <div className="h-7 w-7 rounded-lg bg-gradient-primary grid place-items-center text-[10px] font-bold">{m.sport.slice(0, 2)}</div>
                          <div className="flex-1 min-w-0">
                            <div className="font-medium truncate">{m.title}</div>
                            <div className="text-[10px] text-muted-foreground">{new Date(m.date).toLocaleDateString()}</div>
                          </div>
                          <span className={`text-[8px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wider ${
                            m.status === "Finished" ? "bg-muted text-muted-foreground" :
                            m.status === "IN_PROGRESS" ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" :
                            "bg-primary/20 text-primary-foreground border border-primary/30"
                          }`}>
                            {m.status === "Finished" ? t("profile.status_finished", { defaultValue: "Finalizado" }) :
                             m.status === "IN_PROGRESS" ? t("profile.status_in_progress", { defaultValue: "En Curso" }) :
