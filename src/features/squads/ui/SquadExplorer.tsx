...
import {
  getSquads, joinSquad, leaveSquad, isMember, createSquad, getSquadMembers,
...
import {
  Plus, Users, Loader2, Check, Zap, ArrowLeft, MessageSquare, Calendar, Search,
} from "lucide-react";
...
export function SquadExplorer() {
    ...
    async function loadSquads() {
      try {
        ...
        if (active) {
          ...
          await Promise.all(data.map(async (squad) => {
  ...
  useEffect(() => {
    if (!selectedSquad) { setSquadMembers([]); return; }
    let active = true;
  ...
  useEffect(() => {
    if (!currentUser) return;
    apiClient.users.getMatches().then((users) => setAllSystemUsers(users))
      .catch((err) => console.error("Error fetching match users:", err));
    setLoadingCourts(true);
    backendApi.courts.getAll().then((courtsList) => setSquadCourts(courtsList as Court[]))
      .catch(() => { apiClient.courts.getAll().then((courtsList) => setSquadCourts(courtsList))
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "squads" }, (payload) => {
        const newRow = payload.new as { id: string; name: string; description: string | null; created_at: string; creator_id: string; avatar_url: string | null };
        if (newRow.creator_id === currentUser.id) return;
        const newSquad: Squad = { id: newRow.id, name: newRow.name, description: newRow.description, created_at: newRow.created_at, creator_id: newRow.creator_id, avatar_url: newRow.avatar_url, members_count: 1 };
        setSquads((prev) => { if (prev.some((s) => s.id === newSquad.id)) return prev; return [newSquad, ...prev]; });
  ...
  const handleToggleJoin = async (squadId: string, event?: React.MouseEvent) => {
    ...
    setSquads((prev) => prev.map((s) =>
      s.id === squadId ? { ...s, members_count: isCurrentlyMember ? Math.max(0, (s.members_count ?? 1) - 1) : (s.members_count ?? 0) + 1 } : s
    ));

    try {
      if (isCurrentlyMember) { await leaveSquad(squadId, currentUser.id); toast.success("Has salido del Squad"); }
      else { await joinSquad(squadId, currentUser.id); toast.success("¡Te has unido al Squad!"); }
    } catch (error) {
      // Rollback en caso de error.
      setMemberships((prev) => ({ ...prev, [squadId]: isCurrentlyMember }));
      setSquads((prev) => prev.map((s) =>
        s.id === squadId ? { ...s, members_count: isCurrentlyMember ? (s.members_count ?? 0) + 1 : Math.max(0, (s.members_count ?? 1) - 1) } : s
      ));
  ...
  const handleCreateSquad = async (e: React.FormEvent) => {
    ...
    } catch (err) { console.error("Failed to create squad:", err); toast.error("Error al crear el Squad"); }
  ...
  const handleOpenChat = async () => {
    ...
    if (!existing) {
      chatStore.chats.push({
        id: chatId, name: selectedSquad.name, avatar: "👥",
        current_players: [currentUser.id, ...squadMembers.map((m) => m.id)],
        messages: [{ id: `msg_system_${Date.now()}`, sender_id: "system", text: `Chat grupal iniciado para el Squad "${selectedSquad.name}".`, created_at: new Date().toISOString() }],
        unread: 0,
      });
    }
  ...
  const handleInviteUser = async (user: User) => {
    ...
    try {
      ...
      await createNotification(user.id, "SQUAD_INVITE", `Invitación de Squad`,
        `${currentUser.name} te ha invitado a unirte a su Squad "${selectedSquad.name}".`, `/app/chat`);
      toast.success(`Invitación enviada con éxito a ${user.name}`);
      const list = await getSquadMembers(selectedSquad.id);
      setSquadMembers(list);
    } catch (err) { console.error("Failed to invite user:", err); toast.error("Error al enviar la invitación."); }
  };
  ...
  if (selectedSquad) {
    ...
    const filteredUsersToInvite = allSystemUsers.filter(
      (u) => u.id !== currentUser.id && !squadMembers.some((m) => m.id === u.id) &&
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-card border border-border rounded-3xl p-6 shadow-card">
          <div className="flex gap-4 items-center">
            <button onClick={() => setSelectedSquad(null)}
              className="p-2 rounded-xl bg-accent text-accent-foreground hover:bg-accent/80 transition-colors border border-border" title="Volver">
              <ArrowLeft className="h-4 w-4" />
            </button>
            <img src={selectedSquad.avatar_url || "https://api.dicebear.com/7.x/identicon/svg"} alt=""
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => handleToggleJoin(selectedSquad.id)}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border ${
                isUserMember ? "bg-accent text-accent-foreground border-border hover:bg-accent/80" : "bg-gradient-neon text-neon-foreground shadow-neon border-transparent hover:scale-105"
              }`}>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 bg-gradient-card border border-border rounded-3xl p-5 shadow-card space-y-4 flex flex-col h-[500px]">
            <div className="flex justify-between items-center pb-2 border-b border-border/50">
              ...
              {isUserMember && (
                <button onClick={() => { setInviteSearch(""); setIsInviteModalOpen(true); }}
            ...
            {loadingMembers ? (
              <div className="flex-1 flex items-center justify-center"><Loader2 className="h-5 w-5 animate-spin text-electric" /></div>
            ) : (
              <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                {squadMembers.map((member) => (
                  <div key={member.id} className="flex items-center gap-3 p-2 rounded-xl bg-background/40 border border-border/40">
                    <img src={member.avatar_url} alt={member.name} className="h-8 w-8 rounded-full bg-muted object-cover border border-border" />
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-xs truncate">{member.name}</div>
                      <div className="text-[10px] text-muted-foreground truncate">{member.city || "Lima, PE"}</div>
                    </div>
                    {member.id === selectedSquad.creator_id && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded bg-primary/20 text-primary font-bold uppercase tracking-wider shrink-0">Fundador</span>
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="md:col-span-2 bg-gradient-card border border-border rounded-3xl p-5 shadow-card space-y-4 flex flex-col h-[500px]">
            <div className="pb-2 border-b border-border/50">
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">
                Al reservar desde este panel, el costo se dividirá automáticamente en partes iguales entre todos los miembros activos del Squad.
              </p>
            </div>
            {loadingCourts ? (
              <div className="flex-1 flex items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-electric" /></div>
            ) : (
                  ...
                  return (
                    <div key={court.id}
                      ...
                      <div className="flex gap-3">
                        <img src={court.image_url} alt={court.name} className="h-14 w-20 rounded-xl object-cover border border-border" />
                        <div>
                          <h4 className="font-semibold text-xs text-foreground leading-snug">{court.name}</h4>
                          <p className="text-[10px] text-muted-foreground mt-0.5">{court.sport} · {court.address}</p>
                          <div className="flex items-center gap-3 mt-1.5">
                            <span className="text-[10px] text-muted-foreground">Cancha: S/ {court.price_per_hour}/h</span>
                            <span className="text-[10px] text-neon font-bold">Tu parte: S/ {Math.ceil(priceSplit)} FC</span>
                          </div>
                        </div>
                      </div>
                      <button onClick={() => setSelectedCourtForBooking(court)} disabled={!isUserMember}
            ...
            <div className="relative w-full max-w-md bg-gradient-card border border-border rounded-3xl p-6 shadow-card overflow-hidden flex flex-col max-h-[80vh] z-10 animate-scale-up">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setIsInviteModalOpen(false)}
                  className="h-8 w-8 rounded-full bg-muted grid place-items-center hover:bg-accent transition-colors cursor-pointer text-xs">✕</button>
              </div>
              <div className="relative mb-4">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <input type="text" placeholder="Buscar deportistas..." value={inviteSearch}
              ...
              <div className="flex-1 overflow-y-auto space-y-2 pr-1 min-h-[250px]">
                {filteredUsersToInvite.length === 0 ? (
                  <div className="text-xs text-muted-foreground py-10 text-center">No se encontraron deportistas disponibles.</div>
                ) : (
                  filteredUsersToInvite.map((u) => (
                    <div key={u.id}
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        <img src={u.avatar_url} alt={u.name} className="h-9 w-9 rounded-full bg-muted object-cover border border-border" />
                        <div className="min-w-0">
                          <div className="font-semibold text-xs truncate">{u.name}</div>
                          <div className="text-[10px] text-muted-foreground truncate">{u.bio || "Deportista"}</div>
                        </div>
                      </div>
                      <button onClick={() => handleInviteUser(u)}
                        className="px-3 py-1.5 rounded-lg bg-neon text-neon-foreground text-[10px] font-bold shadow-neon hover:scale-105 transition-transform">Invitar</button>
                    </div>
                  ))
                )}
              </div>
        ...
        <BookingModal court={selectedCourtForBooking} isOpen={selectedCourtForBooking !== null}
          onOpenChange={(open) => !open && setSelectedCourtForBooking(null)} baseLocation={null}
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea value={description} onChange={(e) => setDescription(e.target.value)}
                  ...
                  id="squad-desc-input" required />
              </div>
            </div>
            <div className="flex justify-end gap-2 text-xs pt-2">
              <button type="button" onClick={() => setShowCreateForm(false)}
                className="px-3 py-2 rounded-xl border border-border bg-card text-muted-foreground hover:bg-accent">Cancelar</button>
        ...
        <div className="grid sm:grid-cols-2 gap-4" id="squads-list">
          <AnimatePresence>
            {squads.map((squad) => {
              ...
              return (
                <motion.div key={squad.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0 }} onClick={() => setSelectedSquad(squad)}
                  ...
                  <div>
                    <div className="flex gap-3">
                      <img src={squad.avatar_url || "https://api.dicebear.com/7.x/identicon/svg"} alt=""
                        className="h-12 w-12 rounded-xl bg-card border border-border p-1 group-hover:scale-105 transition-transform" />
                      <div className="flex-1 min-w-0">
                        <h4 className="font-semibold text-sm truncate group-hover:text-neon transition-colors">{squad.name}</h4>
                  ...
                  <div className="mt-4 pt-3 border-t border-border/50 flex justify-between items-center">
                    <span className="text-[10px] text-electric font-semibold opacity-0 group-hover:opacity-100 transition-opacity">Ver Panel →</span>
                    <button onClick={(e) => handleToggleJoin(squad.id, e)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all active:scale-95 cursor-pointer ${
                        joined ? "bg-accent text-accent-foreground border border-border hover:bg-accent/85" : "bg-gradient-neon text-neon-foreground shadow-neon hover:scale-105"
                      }`} id={`join-btn-${squad.id}`}>
                      {joined ? <><Check className="h-3.5 w-3.5" /> Miembro</> : <><Plus className="h-3.5 w-3.5" /> Unirme</>}
                    </button>
                  </div>
                </motion.div>
              );
