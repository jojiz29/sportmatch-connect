...
const createCourtIcon = (sport: string, isSponsored?: boolean) => {
  ...
  const getSportEmoji = (name: string) => {
    switch (name.toLowerCase()) {
      case "paddle": case "padel": case "pádel": return "🏓";
      case "football": case "futbol": case "fútbol": return "⚽";
      case "tennis": case "tenis": return "🎾";
      case "running": return "🏃";
      case "basketball": case "basquet": case "básquet": return "🏀";
      case "volleyball": case "voley": case "vóley": return "🏐";
      default: return "🏆";
    }
  };
  const emoji = getSportEmoji(sport);
  const border = isSponsored ? "3px solid #fbbf24" : "2.5px solid #ffffff";
  const bg = isSponsored ? "linear-gradient(135deg, #f59e0b, #d97706)" : "linear-gradient(135deg, #8b5cf6, #3b82f6)";
  const shadow = isSponsored ? "0 0 20px rgba(251, 191, 36, 0.9), 0 0 8px rgba(251, 191, 36, 0.5)" : "0 0 15px rgba(139, 92, 246, 0.7)";
...
const createBusinessIcon = (isSponsored?: boolean, category?: string) => {
  ...
  const shadow = isSponsored ? "0 0 15px rgba(251, 191, 36, 0.9)" : "0 0 10px rgba(16, 185, 129, 0.5)";
  const bg = isSponsored ? "linear-gradient(135deg, #f59e0b, #d97706)" : "linear-gradient(135deg, #10b981, #059669)";
...
function PopupContent({ venue, onBookCourt, onViewCourt, owner, onViewCommercialSheet }: PopupContentProps) {
  ...
  return (
    <div ref={containerRef} className="p-1 min-w-[200px] max-w-[240px] font-sans text-left">
      ...
      {venue.description && (
        <p className="text-[11px] text-muted-foreground line-clamp-2 mb-1.5 leading-snug">{venue.description}</p>
      )}
      <div className="text-[11px] text-muted-foreground mb-2 leading-tight">
        📍 {venue.address || "Dirección no especificada"}{venue.district && ` · ${venue.district}`}
      </div>
      ...
      {owner && (
        <div className="border-t border-border/40 pt-2 mt-2 mb-2">
          <div className="text-[9px] text-muted-foreground font-bold uppercase tracking-wider mb-1.5">Administrado por:</div>
          <div className="flex items-center gap-2 mb-2.5">
            {owner.avatar_url && (
              <img src={owner.avatar_url} alt={owner.company_name || owner.name}
                className="h-6 w-6 rounded-full object-cover shrink-0 border border-border/40" />
            )}
            <div className="min-w-0 flex-1">
              <div className="text-[11px] font-semibold truncate text-foreground">{owner.company_name || owner.name}</div>
...
function BusinessPopup({ player, onViewCommercialSheet }: BusinessPopupProps) {
  ...
  useEffect(() => {
    ...
    const handleNativeClick = (e: MouseEvent) => {
      ...
      if (action === "profile") { e.preventDefault(); e.stopPropagation(); onViewCommercialSheet?.(player); }
    };
    el.addEventListener("click", handleNativeClick);
    return () => el.removeEventListener("click", handleNativeClick);
  ...
  return (
    <div ref={containerRef} className="p-1 font-sans min-w-[150px] text-left">
      {isBusiness && player.is_sponsored && (
        <span className="inline-block text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-amber-500/20 text-amber-400 border border-amber-500/30 mb-1.5 animate-pulse">⭐ Sponsor Premium</span>
      )}
      <div className="font-bold text-sm text-foreground">{player.company_name || player.name}</div>
      <div className="text-xs text-muted-foreground mt-0.5">
        {isBusiness ? `${player.business_category} · ` : ""}{player.distance_km !== undefined ? `${player.distance_km.toFixed(1)} km` : "Cerca de ti"}
      </div>
      {isBusiness && (
...
export function MapFeature({
  ...
  const onViewCourt = useCallback((courtId: string) => {
    ...
    return players.map((m) => {
      ...
      return (
        <Marker key={m.id} position={[m.last_location_lat, m.last_location_lng]} icon={icon}>
          <Popup><BusinessPopup player={m} onViewCommercialSheet={onViewCommercialSheet} /></Popup>
        </Marker>
      );
    });
  ...
  return (
    <div className="h-[600px] min-h-[500px] w-full flex-1 rounded-3xl overflow-hidden border border-border shadow-card relative z-0">
      <MapContainer center={[-12.14, -76.995]} zoom={13}
        style={{ height: "100%", width: "100%", minHeight: "500px" }}>
        <TileLayer className="map-tiles-dark"
          attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a>'
          url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
        {selectedDistrictCenter && <MapController center={mapCenter} zoom={mapZoom} />}
        {highlightCoords && (
          <Circle center={highlightCoords} radius={700}
            pathOptions={{ color: "#39FF14", fillColor: "#39FF14", fillOpacity: 0.15, weight: 2.5, dashArray: "6, 6" }} />
        )}
        {matchMarkers}
        {venueMarkers}
      </MapContainer>
