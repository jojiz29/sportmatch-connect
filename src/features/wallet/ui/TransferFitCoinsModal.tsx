...
export function TransferFitCoinsModal({ open, onClose }: TransferFitCoinsModalProps) {
  ...
  const [players, setPlayers] = useState<
    Pick<User, "id" | "name" | "avatar_url" | "fitcoins_balance">[]
  >([]);
  const [loadingPlayers, setLoadingPlayers] = useState(false);
  const [selectedPlayer, setSelectedPlayer] = useState<Pick<
    User,
    "id" | "name" | "avatar_url" | "fitcoins_balance"
  > | null>(null);
  ...
  const { values, handleChange, handleBlur, handleSubmit, errors, isSubmitting, setValues } =
    useStrictForm<TransferFormValues>({
      ...
      validate: (vals) => {
        ...
        if (!vals.amount || isNaN(amountNum) || amountNum <= 0)
          errs.amount = "El monto debe ser mayor a 0.";
        if (amountNum > balance)
          errs.amount = `Saldo insuficiente. Tenés ${balance} FC disponibles.`;
            ...
            <div className="px-6 py-5">
              ...
              {step === "success" && (
                ...
                >
                  ...
                  <button
                    onClick={onClose}
                    className="mt-2 px-8 py-2.5 rounded-xl bg-gradient-neon text-neon-foreground font-semibold hover:shadow-neon transition-shadow cursor-pointer"
                    id="transfer-success-close-btn"
                  >
                    Listo
                  </button>
                </motion.div>
              ...
              {step !== "success" && (
                <form onSubmit={handleSubmit} noValidate>
                      ...
                      <div className="flex items-center gap-3 p-3 rounded-xl bg-primary/8 border border-primary/30">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-muted-foreground">
                            {selectedPlayer.fitcoins_balance ?? "—"} FC
                          </div>
                        </div>
                        <button
                          type="button"
                    ...
                    {!selectedPlayer && (
                      <AnimatePresence>
                        <motion.div
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          exit={{ opacity: 0, y: -6 }}
                          className="mt-2 max-h-44 overflow-y-auto rounded-xl border border-border bg-card shadow-lg"
                          id="transfer-player-list"
                        >
                          {loadingPlayers ? (
                    ...
                    {parsedAmount > 0 && (
                      <div className="mt-2">
                        ...
                        <div className="flex justify-between text-[10px] text-muted-foreground mt-0.5">
                          <span>0 FC</span>
                          <span
                            className={amountExceedsBalance ? "text-destructive font-semibold" : ""}
                          >
                            {amountExceedsBalance
                              ? "Saldo insuficiente"
                              : `Restante: ${balance - parsedAmount} FC`}
                          </span>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      type="submit"
                      disabled={isSubmitting || amountExceedsBalance || !selectedPlayer}
