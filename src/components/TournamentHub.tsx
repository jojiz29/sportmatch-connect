...
export function TournamentHub() {
  ...
  // Sprint 3.6 Advanced Integration: Squad panel & Championship codes
  ...
  // Squad Members State - populated dynamically (not hardcoded) using real BBDD / mock stack
  ...
  // Dynamically resolve starters & subs limit based on selected sport and tactical format (Task 2.3)
  const activeConfig = useMemo(() => {
    ...
    return {
      startersLimit: startersCount,
      subsLimit: config.subs,
    };
  ...
  // Brackets matches structure representing Quarter-finals (4 nodes), Semi-finals (2 nodes), Finals (1 node)
  ...
  // Search results querying standard BBDD users dynamically (Task 2.3)
  ...
  // Invite people by sending a message dynamically through the chat store (Task 2.3)
  const handleInviteUserByChat = async (targetUser: User) => {
    try {
      ...
      // 1. Resolve or create chat thread
  ...
  const handleSimulateStep = () => {
    if (simulationStep === "init") {
      ...
      setBrackets((prev) => ({ ...prev, quarters: newQuarters, semis: nextSemis }));
      ...
      setBrackets((prev) => ({ ...prev, semis: newSemis, finals: nextFinals }));
      ...
      setBrackets((prev) => ({ ...prev, finals: finishedFinals }));
      setChampion(winnerTeam);
      setSimulationStep("finals");
      toast.success(`¡El torneo ha terminado! Campeón: ${winnerTeam.name}`, {
        icon: "🏆",
      });
    }
  };
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      {activeTab === "squads" ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
          {/* LEFT PANEL: COLLAPSIBLE SQUAD MANAGEMENT (Sprint 3.6 Overhaul) */}
          <div className="col-span-1 bg-gradient-card border border-border rounded-3xl p-4 shadow-card space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-white/5">
              <h3 className="font-heading text-lg tracking-wide text-foreground flex items-center gap-2">
                <Users className="h-4.5 w-4.5 text-neon" />
                Mi Escuadra
              </h3>
              <button
            ...
            {isSquadPanelOpen && (
              <div className="space-y-4 animate-scale-in text-left">
                {/* Dynamic Sport and Format Toggles */}
                  ...
                  <div className="grid grid-cols-3 gap-1 bg-background/40 p-1 border border-border/50 rounded-xl">
                    {(["Fútbol", "Vóley", "Pádel"] as const).map((s) => (
                      <button
                        ...
                        className={`py-1 rounded-lg text-[10px] font-black tracking-wide transition-all cursor-pointer ${
                          selectedSport === s
          ...
          {/* CENTER PANEL: CHAMPIONSHIP BRACKETS & CONTROLLER */}
          <div className="col-span-1 lg:col-span-2 space-y-6">
            {/* Join Code Controller */}
            ...
            >
              <div className="min-w-[900px] flex justify-between items-center gap-6 py-6 relative">
                ...
                <div className="flex-1 space-y-24">
                  ...
                  {/* Champion crowning slot (glowing gold) */}
                  {champion && (
          ...
          {/* RIGHT PANEL: USER SEARCH & CHAT RECRUITMENT (Sprint 3.6 Overhaul) */}
          <div className="col-span-1 bg-gradient-card border border-border rounded-3xl p-4 shadow-card space-y-4">
            <div className="pb-2 border-b border-white/5 text-left">
              <h3 className="font-heading text-lg tracking-wide text-foreground flex items-center gap-2">
                <UserPlus className="h-4.5 w-4.5 text-neon" />
                Reclutar por Chat
              </h3>
      ...
      {/* Matchup Versus Dialog Screen (Task 2.4) */}
      <MatchupVersus
        isOpen={isVersusOpen}
...
// Single match bracket rendering component
