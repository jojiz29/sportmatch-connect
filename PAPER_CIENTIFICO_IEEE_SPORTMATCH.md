...
- **Null Hypothesis ($H_0$):** User weekly match frequency does not increase ($\mu_{\text{post}} \le \mu_{\text{pre}}$).
- **Alternative Hypothesis ($H_1$):** User weekly match frequency increases significantly ($\mu_{\text{post}} > \mu_{\text{pre}}$).
