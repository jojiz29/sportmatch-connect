...
(async () => {
  ...
  if (sample.rows[0]) {
    ...
    const beforeXp = await c.query("SELECT xp, level FROM profiles WHERE user_id = $1", [uid]);
    ...
    // Reset
    await c.query("UPDATE profiles SET xp = $1 WHERE user_id = $2", [beforeXp.rows[0].xp, uid]);
  }
