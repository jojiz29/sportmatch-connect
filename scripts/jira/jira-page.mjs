  ...
  if (i.fields.sprint && Array.isArray(i.fields.sprint)) {
    i.fields.sprint.forEach((s) => {
      sprintMap.set(s.id, {
        ...
        endDate: s.endDate,
        count: (sprintMap.get(s.id)?.count || 0) + 1,
      });
    });
  } else if (i.fields.sprint && typeof i.fields.sprint === "object") {
    sprintMap.set(i.fields.sprint.id, {
      ...i.fields.sprint,
      count: (sprintMap.get(i.fields.sprint.id)?.count || 0) + 1,
    });
  }
});
...
Array.from(sprintMap.entries())
  ...
  .forEach(([id, s]) => {
    console.log(
      `  Sprint ${id} [${s.state}]: ${s.name} (${s.startDate ? s.startDate.slice(0, 10) : "-"} → ${s.endDate ? s.endDate.slice(0, 10) : "-"}) — ${s.count} issues`,
    );
  });
