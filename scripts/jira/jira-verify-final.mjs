...
newTickets.issues.forEach((i) => {
  ...
  console.log(
    `  ${i.key} [${sprints}] [${i.fields.status.name}] [${i.fields.assignee?.displayName || "none"}]`,
  );
...
histTickets.issues.forEach((i) => {
  console.log(
    `  ${i.key} [${i.fields.status.name}] [${i.fields.assignee?.displayName || "none"}] ${i.fields.summary}`,
  );
});
...
if (others.issues) {
  others.issues.slice(0, 10).forEach((i) => {
    const sprints = (i.fields.customfield_10020 || []).map((s) => s.name).join(",");
    console.log(
      `  ${i.key} [${sprints}] [${i.fields.assignee?.displayName}] ${i.fields.summary.slice(0, 60)}`,
    );
  });
}
