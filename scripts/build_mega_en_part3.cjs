...
console.log("Adding English Mega Cap III (Design Thinking, Lean Startup, BMC, Financials)...");

append("# CHAPTER III: TECHNICAL AND BUSINESS METHODOLOGY\n");
append("## i. Methodology (Hybrid Framework)\n");
append("The project adopts a structured hybrid methodology combining **Design Thinking** for human-centered problem discovery, **Lean Startup** for MVP validation and Build-Measure-Learn feedback loops, and the **Scrum** framework (complemented by Kanban) for software engineering sprints.\n");
append("It is academically vital to clarify that **Scrum IS NOT a methodology**, but an adaptive and lightweight framework based on empiricism and Lean thinking (Schwaber & Sutherland, 2020).\n");
