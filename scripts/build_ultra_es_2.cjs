...
console.log("Añadiendo Cap I y Cap II ultra extensos con ecuaciones GitHub Math en TESIS_FINAL_SPORTMATCH_ES.md...");
...
append("El motor de matchmaking predictivo implementa una función de compatibilidad multivariable ponderada en el rango [0, 100], disenada para maximizar la probabilidad de satisfaccion mutua entre rivales o companeros de equipo:\n");
...
append("Donde las ponderaciones satisfacen estrictamente la restriccion de normalizacion algebraico dada por $\\sum_{i=1}^{5} w_i = 1.0$:\n");
append("- $w_1 = 0.35$ (Cercania geografica mediante la formula ortodromica de Haversine).\n");
append("- $w_2 = 0.30$ (Coincidencia exacta de deporte preferido — filtro binario estricto).\n");
append("- $w_3 = 0.20$ (Similitud de nivel de destreza basado en el algoritmo de rating Elo).\n");
append("- $w_4 = 0.10$ (Solapamiento de franjas horarias de disponibilidad semanal).\n");
append("- $w_5 = 0.05$ (Trust Score o reputacion auditada del perfil de usuario).\n");

append("### Fórmula de Distancia Ortodrómica (Haversine)\n");
append("Para calcular la distancia exacta sobre la superficie terrestre en kilometros entre la posicion del usuario $A(\\phi_1, \\lambda_1)$ y la cancha o rival candidato $B(\\phi_2, \\lambda_2)$:\n");
append("\n$$\na = \\sin^2\\left(\\frac{\\Delta\\phi}{2}\\right) + \\cos(\\phi_1)\\cos(\\phi_2)\\sin^2\\left(\\frac{\\Delta\\lambda}{2}\\right)\n$$\n");
append("\n$$\nc = 2 \\cdot \\operatorname{atan2}\\left(\\sqrt{a}, \\sqrt{1-a}\\right)\n$$\n");
append("\n$$\nd = R \\cdot c\n$$\n");
append("Donde $R = 6371\\text{ km}$ representa el radio medio terrestre. Posteriormente, el score de cercania espacial se normaliza exponencialmente mediante la siguiente funcion:\n");
append("\n$$\nS_{\\text{cercanía}} = 100 \\times \\max\\left(0, 1 - \\frac{d}{d_{\\max}}\\right) \\quad \\text{donde } d_{\\max} = 50\\text{ km}\n$$\n");

console.log("Parte 2 ultra completada.");
