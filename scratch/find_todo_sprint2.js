...
async function main() {
  try {
    const sprintId = 41;
    const jql = `project = SCRUM AND sprint = ${sprintId} AND status != "Finalizada"`;
    const url = `${baseUrl}/rest/api/3/search/jql?jql=${encodeURIComponent(jql)}&maxResults=100&fields=summary,status,assignee`;
    ...
    console.log(`Found ${data.issues.length} non-finalized issues in Sprint 2:`);
